...
class Field():
    ...
    def __init__(self, container, item_code, rows, columns):
    ...
    def __str__(self):
        return stashvar.atm_stashvar[self.item_code][0]

    def __getitem__(self, index):

        # Read in data on first read. 
        if self.data is None:
            self.data = self.container.readfld(self.item_code)

        return self.data[index]

    def __setitem__(self, index, value):

        # Read in data to initialise. 
        if self.data is None:
            self.data = self.container.readfld(self.item_code)
    ...
    def write(self):
        ...
        if self.data is not None:
            self.container.writefld(self.data, self.item_code)
...
class UMFile(file):
    ...
    def __init__(self, *args):
        ...
        # Set up 'fields' dictionary.
        self.setup_fields()


    def setup_fields(self):
        """
        Cycle through the fields and create a dict to access each.
        """

        for i in range(self.fixhd[FH_LookupSize2]):
            item_code = self.ilookup[i][ITEM_CODE]
            rows = self.ilookup[i][LBROW]
            cols = self.ilookup[i][LBNPT]

            self.fields[item_code] = Field(self, item_code, rows, cols)
        
    def close(self):

        # Write out all the field data.
